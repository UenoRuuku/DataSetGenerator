...
def main():
    ...
    os.mkdir(folder+"integrated_dataset/rate101")
    # all_train_list = []
    # all_valid_list = []
    # all_test_list = []
    # print(len(all_train_list), len(all_valid_list), len(all_test_list))
    ...
    for file_name in file_names:
        ...
        if file_name.startswith('memc_') or file_name.startswith('.') or file_name.startswith('all_') or categ_name in completed_categ:
            ...
            # extent dataset list
            all_train_list.extend( cate_dataset[:int(len_cate_dataset *6/9)] )
            all_valid_list.extend( cate_dataset[int(len_cate_dataset *6/9) : int(len_cate_dataset *7/9)] )
            all_test_list.extend( cate_dataset[int(len_cate_dataset *7/9) : ] )
            print(len(all_train_list), len(all_valid_list), len(all_test_list))
            ...
            File_processing.write_TXTfile(folder + 'integrated_dataset/rate101' +categ_name + '_train_rate101.txt', cate_train)
            File_processing.write_TXTfile(folder + 'integrated_dataset/rate101' +categ_name + '_valid_rate101.txt', cate_valid)
            File_processing.write_TXTfile(folder + 'integrated_dataset/rate101' +categ_name + '_test_rate101.txt', cate_test)
